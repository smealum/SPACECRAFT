...
void main()
{
	...
	vec3 vt=n*m_fInnerRadius;

	// //we want to do a reflection
	// //at this point vt is the point on the inner sphere
	// //we want to reflect the ray going from cameraPosition to vt and get its intersection with the outer sphere
	// //  cp    vt2
	// //   \    /
	// //    \  /
	// // ____\/____
	...
	// vec3 u=cameraPosition-vt;
	// vec3 u2=u-2*dot(u,n)*n;
	// vec3 cp2=vt+u2;
	// u2=-u2;

	// //formule de http://en.wikipedia.org/wiki/Line%E2%80%93sphere_intersection
	// float d1=dot(u2,cp2);
	// float d=-d1+sqrt(m_fOuterRadius*m_fOuterRadius-dot(cp2,cp2)+d1*d1);
	// vt=cp2+u2*d;
	// vec3 t=SetColor(vt, vt, lightDirection);
