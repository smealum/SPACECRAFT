...
void CameraKeyboardMouse::update(Camera& camera) {
  if (Input::isMouseFixed()) {
    ...
    // mouse speed
    const float s = 0.002;
    camera.view3 = mat3(rotate(mat4(1.0), s * Input::getHorAngle(),
                               vec3(0.0, -1.0, 0.0))) *
                   camera.view3;
    camera.view3 = mat3(rotate(mat4(1.0), s * Input::getVerAngle(),
                               vec3(-1.0, 0.0, 0.0))) *
                   camera.view3;
  ...
  Planet* planet =
      globalGalaxy->getClosestPlanet(camera.getPosition(glm::vec3(0)));
  if (planet)
